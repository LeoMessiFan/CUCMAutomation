...
def _is_e164(dn: str) -> bool:
    """
    Returns True only if the user explicitly entered a \\+ or + prefix.
    Internal extensions (e.g. 60011) return False.
    E.164 numbers (e.g. \\+19786007919 or +19786007919) return True.
    """
    return dn.startswith("\\+") or dn.startswith("+")


def _search_pattern(dn: str) -> str:
    """Build AXL wildcard search pattern."""
...
def _full_dn(dn: str) -> str:
    """
    Build full DN for AXL getLine / updateUser calls.
    E.164 numbers get \\+ prefix. Internal extensions are returned as-is.
    """
    if _is_e164(dn):
        digits = _normalize_dn(dn)
        return "\\+" + digits
